<?php $__env->startSection('pageTitle', 'Kanban de tarefas'); ?>
    ...
    <div class="rounded-xl border border-gray-200 bg-slate-50/80">
        <div class="max-h-[60vh] overflow-auto p-3 md:max-h-[calc(100vh-260px)] md:p-4">
            <div id="kanban-board"
                ...
                data-task-update-url-template="<?php echo e(route('tasks.update', ['task' => '__TASK_ID__'])); ?>"
                ...
                <div class="flex min-h-[340px] flex-col rounded-lg border border-gray-200 bg-white shadow-sm">
                    <div class="flex items-center justify-between gap-2 border-b border-gray-100 bg-gray-50/80 px-3 py-2">
                        ...
                        <span class="kanban-count text-xs text-gray-400" data-status="<?php echo e($statusKey); ?>">
                            <?php echo e(isset($tasksByStatus[$statusKey]) ? $tasksByStatus[$statusKey]->count() : 0); ?>

                        </span>
                    </div>

                    <div class="kanban-column flex-1 space-y-3 overflow-y-auto p-3" data-status="<?php echo e($statusKey); ?>">
                        <?php $__empty_1 = true; $__currentLoopData = ($tasksByStatus[$statusKey] ?? collect()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $task): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="kanban-card cursor-move rounded-md border border-gray-200 bg-white p-3 shadow-sm transition-all hover:border-indigo-300 hover:shadow-md"
                            ...
                            data-task-status-label="<?php echo e($statuses[$task->status]['label'] ?? $statusConfig['label']); ?>"
                            data-task-priority="<?php echo e($task->priority); ?>"
                            data-task-priority-label="<?php echo e($priorityOptions[$task->priority] ?? 'Media'); ?>"
                            data-task-due-date="<?php echo e(optional($task->due_date)->format('d/m/Y')); ?>"
                            data-task-due-date-raw="<?php echo e(optional($task->due_date)->format('Y-m-d')); ?>"
                            data-task-assignee-id="<?php echo e(optional($task->users->first())->id); ?>"
                            data-task-assignee="<?php echo e(($task->users && $task->users->count()) ? $task->users->pluck('name')->join(', ') : 'Sem responsavel'); ?>"
                            data-task-process-id="<?php echo e($task->processo?->id); ?>"
                            data-task-process-number="<?php echo e($task->processo?->numero_processo); ?>"
                            data-task-process-label="<?php echo e($task->processo ? trim($task->processo->numero_processo . ' - ' . ($task->processo->customer?->name ?? $task->processo->tribunal)) : 'Sem processo vinculado'); ?>"
                            data-task-created="<?php echo e(optional($task->created_at)->format('d/m/Y H:i')); ?>"
                            data-task-updated="<?php echo e(optional($task->updated_at)->format('d/m/Y H:i')); ?>">
                            <div class="flex items-start justify-between gap-2">
                                <div class="min-w-0">
                                    ...
                                    <?php if($task->processo): ?>
                                    <p class="mt-1 truncate text-[11px] text-slate-500">
                                        Processo: <?php echo e($task->processo->numero_processo); ?>
                            ...
                            <div class="mt-3 flex flex-wrap items-center gap-2 text-[11px]">
                                <span class="inline-flex items-center rounded-full px-2 py-0.5 font-medium <?php echo e($priorityStyles[$task->priority] ?? $priorityStyles['medium']); ?>">
                                    <?php echo e($priorityOptions[$task->priority] ?? 'Media'); ?>

                                </span>
                                <span class="inline-flex items-center rounded-full bg-gray-100 px-2 py-0.5 text-gray-600">
                                    Prazo: <?php echo e($task->due_date ? $task->due_date->format('d/m/Y') : 'Nao definido'); ?>
                            ...
                            <div class="mt-3 flex items-center justify-between gap-2">
                                <span class="kanban-assignee-label inline-flex items-center rounded-full bg-gray-100 px-2 py-0.5 text-[11px] text-gray-600">
                                    <?php if($task->users && $task->users->count()): ?>
                                ...
                                <select
                                    ...
                                    <option value="<?php echo e($user->id); ?>" <?php if($task->users && $task->users->contains($user)): echo 'selected'; endif; ?>>
                ...
                <form action="<?php echo e(route('tasks.store')); ?>" method="POST" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select name="datajud_processo_id" id="modal-process"
                                ...
                                <option value="<?php echo e($process->id); ?>" <?php if((string) old('datajud_processo_id') === (string) $process->id): echo 'selected'; endif; ?>>
                                    <?php echo e($process->numero_processo); ?><?php if($process->customer): ?> - <?php echo e($process->customer->name); ?><?php endif; ?>
                                </option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="status" id="modal-status"
                                class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <option value="pending" <?php if(old('status', 'pending') === 'pending'): echo 'selected'; endif; ?>>Pendente</option>
                                <option value="in_progress" <?php if(old('status') === 'in_progress'): echo 'selected'; endif; ?>>Em progresso</option>
                                <option value="completed" <?php if(old('status') === 'completed'): echo 'selected'; endif; ?>>Concluida</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="users[]" id="modal-assignee"
                                ...
                                <?php $__currentLoopData = $users; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($user->id); ?>" <?php if((string) old('users.0') === (string) $user->id): echo 'selected'; endif; ?>><?php echo e($user->name); ?></option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="modal-description" rows="3"
                            class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                            placeholder="Detalhes, proximos passos ou contexto da tarefa"><?php echo e(old('description')); ?></textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select name="datajud_processo_id" id="edit-process"
                                ...
                                <option value="">Sem processo vinculado</option>
                                <?php $__currentLoopData = $processes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $process): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($process->id); ?>">
                                    <?php echo e($process->numero_processo); ?><?php if($process->customer): ?> - <?php echo e($process->customer->name); ?><?php endif; ?>
                                </option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const statusLabels = <?php echo json_encode(collect($statuses)->mapWithKeys(fn ($config, $key) => [$key => $config['label']]), 512) ?>;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\TECNOLOGIA\OneDrive - Faculdade Atenas\Área de Trabalho\juristack\resources\views\tasks\index.blade.php ENDPATH**/ ?>
    ...
    public function rules(): array
    {
        ...
        if ($this->input('type') === Cliente::TYPE_PF) {
            $base['cpf'] = [
                ...
                'size:11',
                new CpfValido,
                Rule::unique('clientes', 'cpf')->where('user_id', $userId)->whereNull('deleted_at')->ignore($clienteId),
            ];
            ...
            $base['cnpj'] = [
                ...
                'size:14',
                new CnpjValido,
                Rule::unique('clientes', 'cnpj')->where('user_id', $userId)->whereNull('deleted_at')->ignore($clienteId),
            ];
        } else {

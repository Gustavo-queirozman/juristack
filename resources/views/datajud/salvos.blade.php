...
<div class="w-full max-w-full">
    ...
    <form method="GET" action="{{ route('datajud.salvos') }}" class="mb-6 flex flex-wrap gap-2 items-end">
        ...
        @if(!empty($busca))
            <a href="{{ route('datajud.salvos') }}" class="px-4 py-2 text-gray-600 text-sm font-medium hover:text-gray-900">Limpar</a>
        @endif
    </form>
    ...
    @if($processos->isEmpty())
        <div class="rounded-xl border border-gray-200 bg-white shadow-sm p-8 text-center">
            ...
            @if(!empty($busca))
                ...
                <a href="{{ route('datajud.salvos') }}" class="text-indigo-600 hover:text-indigo-800 text-sm font-medium mr-2">Limpar filtro</a>
                <a href="{{ route('datajud.index') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-md hover:bg-indigo-700">
                    Nova pesquisa
                </a>
            @else
                            ...
                            <div class="min-w-0">
                                ...
                                <div class="flex flex-wrap items-center gap-2 mt-1">
                                    ...
                                    @if($processo->classe_nome)
                                        <span class="text-xs text-gray-500">{{ $processo->classe_nome }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>

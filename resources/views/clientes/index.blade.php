...
<div class="w-full max-w-full">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <h2 class="text-lg font-semibold text-gray-900 m-0">
            {{ $clientes->total() }} {{ $clientes->total() === 1 ? 'usuário' : 'usuários' }}
        </h2>
    ...
    <form method="GET" action="{{ route('users.index') }}" class="mb-6 flex flex-wrap gap-2 items-end">
                        ...
                        @foreach($clientes as $cliente)
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <a href="{{ route('users.show', $cliente) }}" class="font-medium text-indigo-600 hover:text-indigo-800">
                                        {{ $cliente->nome }}
                                    </a>
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-right">
                                    <span class="inline-flex items-center gap-1">
                                        <a href="{{ route('users.show', $cliente) }}" class="p-1.5 text-gray-500 hover:text-indigo-600 rounded hover:bg-gray-100" title="Ver">
                                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                        </a>
                                        <a href="{{ route('users.edit', $cliente) }}" class="p-1.5 text-gray-500 hover:text-indigo-600 rounded hover:bg-gray-100" title="Editar">

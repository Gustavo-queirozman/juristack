...
<div class="max-w-2xl">
    ...
    <form method="POST" action="{{ route('clientes.update', $cliente) }}" class="space-y-6">
            ...
            <div class="p-4 space-y-4">
                <div>
                    ...
                    <select name="type" id="type" required
                        ...
                        <option value="PF" {{ old('type', $cliente->type) === 'PF' ? 'selected' : '' }}>Pessoa Física (PF)</option>
                        <option value="PJ" {{ old('type', $cliente->type) === 'PJ' ? 'selected' : '' }}>Pessoa Jurídica (PJ)</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="nome" id="nome" value="{{ old('nome', $cliente->nome) }}" required maxlength="255"
                ...
                <div id="wrap-cpf" class="{{ old('type', $cliente->type) === 'PJ' ? 'hidden' : '' }}">
                    <label for="cpf" class="block text-sm font-medium text-gray-700 mb-1">CPF <span class="text-red-500">*</span></label>
                    <input type="text" name="cpf" id="cpf" value="{{ old('cpf', $cliente->cpf ? \App\Models\Cliente::formatarCpf($cliente->cpf) : '') }}" maxlength="14"
                ...
                <div id="wrap-cnpj" class="{{ old('type', $cliente->type) === 'PF' ? 'hidden' : '' }}">
                    <label for="cnpj" class="block text-sm font-medium text-gray-700 mb-1">CNPJ <span class="text-red-500">*</span></label>
                    <input type="text" name="cnpj" id="cnpj" value="{{ old('cnpj', $cliente->cnpj ? \App\Models\Cliente::formatarCnpj($cliente->cnpj) : '') }}" maxlength="18"
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $cliente->email) }}" maxlength="255"
                ...
                <div>
                    ...
                    <input type="text" name="telefone" id="telefone" value="{{ old('telefone', $cliente->telefone) }}" maxlength="20"
        ...
        @php $end = $cliente->enderecos->first(); @endphp
        <div class="rounded-lg border border-gray-200 bg-white shadow-sm overflow-hidden">
            <div class="px-4 py-3 border-b border-gray-200 bg-gray-50">
            ...
            <div class="p-4 space-y-4">
                <div>
                    ...
                    <input type="text" name="logradouro" id="logradouro" value="{{ old('logradouro', $end?->logradouro) }}" maxlength="255"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="numero" id="numero" value="{{ old('numero', $end?->numero) }}" maxlength="20"
                    ...
                    <div>
                        ...
                        <input type="text" name="complemento" id="complemento" value="{{ old('complemento', $end?->complemento) }}" maxlength="100"
                               class="block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm">
                    </div>
                </div>
                <div>
                    <label for="bairro" class="block text-sm font-medium text-gray-700 mb-1">Bairro</label>
                    <input type="text" name="bairro" id="bairro" value="{{ old('bairro', $end?->bairro) }}" maxlength="100"
                           class="block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm">
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        <label for="cidade" class="block text-sm font-medium text-gray-700 mb-1">Cidade</label>
                        <input type="text" name="cidade" id="cidade" value="{{ old('cidade', $end?->cidade) }}" maxlength="100"
                    ...
                    <div>
                        ...
                        <select name="estado" id="estado" class="block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm @error('estado') border-red-500 @enderror">
                            <option value="">UF</option>
                            @foreach(['AC','AL','AP','AM','BA','CE','DF','ES','GO','MA','MT','MS','MG','PA','PB','PR','PE','PI','RJ','RN','RS','RO','RR','SC','SP','SE','TO'] as $uf)
                                <option value="{{ $uf }}" {{ old('estado', $end?->estado) === $uf ? 'selected' : '' }}>{{ $uf }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="cep" id="cep" value="{{ old('cep', $end ? preg_replace('/(\d{5})(\d{3})/', '$1-$2', $end->cep) : '') }}" maxlength="10"

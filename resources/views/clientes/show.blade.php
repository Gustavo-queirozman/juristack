...
<div class="w-full max-w-full">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <a href="{{ route('users.edit', $cliente) }}"
               class="inline-flex items-center gap-1.5 px-3 py-1.5 border border-gray-300 text-gray-700 text-sm font-medium rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                Editar
            </a>
            <form method="POST" action="{{ route('users.destroy', $cliente) }}" class="inline form-delete-cliente">
                @csrf
                @method('DELETE')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        <div class="rounded-lg border border-gray-200 bg-white shadow-sm overflow-hidden">
            ...
            <div class="p-4 space-y-2 text-sm">
                <p class="m-0"><span class="font-medium text-gray-700">Tipo:</span> {{ $cliente->type === 'PF' ? 'Pessoa Física' : 'Pessoa Jurídica' }}</p>
                @if($cliente->documento_formatado)<p class="m-0"><span class="font-medium text-gray-700">CPF/CNPJ:</span> {{ $cliente->documento_formatado }}</p>@endif
                @if($cliente->email)<p class="m-0"><span class="font-medium text-gray-700">E-mail:</span> {{ $cliente->email }}</p>@endif
                @if($cliente->telefone)<p class="m-0"><span class="font-medium text-gray-700">Telefone:</span> {{ $cliente->telefone }}</p>@endif
                @if(!$cliente->documento_formatado && !$cliente->email && !$cliente->telefone)
            ...
            <div class="p-4 space-y-2 text-sm">
                @if($cliente->enderecos->isNotEmpty())
                    @foreach($cliente->enderecos as $endereco)
                        <p class="m-0">{{ $endereco->linha_completa }}</p>
                    @endforeach
                @else

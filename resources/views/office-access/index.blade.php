...
@section('pageTitle', 'Acessos do escritório')
...
<div class="w-full max-w-full">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <h2 class="text-lg font-semibold text-gray-900 m-0">
            {{ $users->total() }} {{ $users->total() === 1 ? 'acesso interno' : 'acessos internos' }}
        </h2>
        <a href="{{ route('office-access.create') }}"
    ...
    <form method="GET" action="{{ route('office-access.index') }}" class="mb-6 flex flex-wrap gap-2 items-end">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($users as $accessUser)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{{ $accessUser->name }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{{ $accessUser->email }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{{ $roleLabels[$accessUser->role] ?? $accessUser->role }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-600">{{ $accessUser->enterprise?->name ?? '—' }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm">
                                @if($accessUser->is_active)
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-right">
                                <span class="inline-flex items-center gap-1">
                                    <a href="{{ route('office-access.edit', $accessUser) }}" class="p-1.5 text-gray-500 hover:text-indigo-600 rounded hover:bg-gray-100" title="Editar">
                                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <form method="POST" action="{{ route('office-access.destroy', $accessUser) }}" class="inline form-delete-access">
                                        @csrf
                                        @method('DELETE')

...
@section('pageTitle', 'Editar acesso do escritório')
...
<div class="max-w-3xl">
    ...
    <form method="POST" action="{{ route('office-access.update', $accessUser) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="rounded-lg border border-gray-200 bg-white shadow-sm overflow-hidden">
            ...
            <div class="p-4 space-y-4">
                ...
                <div>
                    ...
                    <select name="enterprise_id" id="enterprise_id" required
                        ...
                        @foreach($enterprises as $enterprise)
                            <option value="{{ $enterprise->id }}" {{ (int) old('enterprise_id', $accessUser->enterprise_id) === (int) $enterprise->id ? 'selected' : '' }}>
                                {{ $enterprise->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $accessUser->name) }}" required maxlength="255"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $accessUser->email) }}" required maxlength="255"
                    ...
                    <div>
                        ...
                        <select name="role" id="role" required
                            ...
                            @foreach($roleOptions as $role => $label)
                                <option value="{{ $role }}" {{ old('role', $accessUser->role) === $role ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $accessUser->is_active) ? 'checked' : '' }}

...
<div class="w-full max-w-4xl">
    ...
    <div class="rounded-lg border border-gray-200 bg-white shadow-sm overflow-hidden">
        <div class="px-4 py-3 border-b border-gray-200 bg-gray-50 flex flex-wrap items-center justify-between gap-3">
            <h2 class="text-base font-semibold text-gray-900">{{ $document->title }}</h2>
            <div class="flex items-center gap-2">
                @if($document->document_link)
                <a href="{{ route('documents.download', $document->id) }}" class="inline-flex items-center gap-2 px-3 py-2 bg-indigo-600 text-white text-sm font-medium rounded-md hover:bg-indigo-700" title="Baixar PDF">
                ...
                @endif
                <form action="{{ route('documents.destroy', $document->id) }}" method="POST" class="inline" onsubmit="return confirm('Excluir este documento?');">
                    @csrf
                    @method('DELETE')
        ...
        <div class="p-4 space-y-4">
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                <div>
                    ...
                    <dd class="font-medium text-gray-900">{{ \App\Models\Document::TYPES[$document->type] ?? $document->type }}</dd>
                </div>
                <div>
                    <dt class="text-gray-500">Modelo base</dt>
                    <dd class="font-medium text-gray-900">{{ $document->template?->title ?? '—' }}</dd>
                </div>
                <div>
                    <dt class="text-gray-500">Cliente / Relacionado</dt>
                    <dd class="font-medium text-gray-900">
                        @if($document->customer)
                        <a href="{{ route('customers.show', $document->customer) }}" class="text-indigo-600 hover:text-indigo-800">{{ $document->customer->name }}</a>
                        @else
                        —
                        @endif
                    </dd>
                ...
                <div class="sm:col-span-2">
                    ...
                    <dd><a href="{{ $document->form_link }}" target="_blank" rel="noopener" class="text-indigo-600 hover:underline break-all">{{ $document->form_link }}</a></dd>
                </div>
                @endif
            </dl>
            @if(!$document->document_link && $document->template)
            <p class="text-sm text-gray-500">Este documento ainda não possui arquivo gerado. Use o modelo "{{ $document->template->title }}" em Documentos → Preencher e gerar para gerar o PDF.</p>
            @endif
        </div>
    </div>
</div>

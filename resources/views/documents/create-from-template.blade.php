...
@section('pageTitle', 'Novo documento a partir do modelo')
...
<div class="w-full max-w-2xl">
    ...
    <div class="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden mb-4">
        <div class="px-4 py-2 border-b border-gray-200 bg-indigo-50">
            <p class="text-sm font-medium text-indigo-900">Modelo: {{ $template->title }}</p>
            <p class="text-xs text-indigo-700">{{ \App\Models\DocumentTemplate::TYPES[$template->type] ?? $template->type }}</p>
        </div>
    </div>
    ...
    <div class="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        ...
        <form action="{{ route('documents.store') }}" method="POST" class="p-4 space-y-4" enctype="multipart/form-data">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $template->title . ' - ' . now()->format('d/m/Y')) }}" required
            ...
            <div>
                ...
                <input type="url" name="form_link" id="form_link" value="{{ old('form_link') }}"
                       class="block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                       placeholder="https://...">
            </div>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('documents.index') }}" class="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50">
                    Cancelar
                </a>
            </div>
        </form>
